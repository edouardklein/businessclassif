...
# We also put the year (1998 or 1999) in the mapped elements, in order to know in which folder to look
# for the file.
# We also compare the category to the list of known categories, and input information relating to
# supercategories
...
# We save the catogories for which we get good results
...
for C in [c for c in categories if nb_items(c) >= 20]:
    # Now, we create the C_text list. Each element is the content of one text file
    # (in either folder 1999/ or 1998/)
    ...
    # This is almost straight from
    # http://scikit-learn.org/stable/tutorial/text_analytics/working_with_text_data.html
    # We convert the text elements of lists C_text and other_text to vectors of numbers using the
    # tf-idf transform.
    all_text = C_text + other_text
    Y = np.zeros(len(C_text) + len(other_text))  #The Y vector contains the 'answer' :
    # its i-th element is 1 if
